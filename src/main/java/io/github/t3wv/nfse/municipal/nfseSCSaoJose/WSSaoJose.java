...
public class WSSaoJose implements NFSeLogger {
	...
	public NFSeSCSaoJoseEmissaoRetorno enviarEmissaoNFSe(final NFSeSCSaoJoseEmissaoEnvio arquivoEmissao,
			final String filename) throws Exception {
		HttpClient client = HttpClient.newHttpClient();
		this.getLogger().info("Request emissão NFSe com xml: {}", arquivoEmissao.toXml());
		final var response = client.send(this.buildRequest(arquivoEmissao.toXml(), filename),
	...
	public NFSeSCSaoJoseCancelamentoRetorno enviarCancelamentoNFSe(
			final NFSeSCSaoJoseCancelamentoEnvio arquivoCancelamento, final String filename) throws Exception {
		HttpClient client = HttpClient.newHttpClient();
		this.getLogger().info("Request cancelamento NFSe com xml: {}", arquivoCancelamento.toXml());
		final var response = client.send(this.buildRequest(arquivoCancelamento.toXml(), filename),
				HttpResponse.BodyHandlers.ofString());
		final var responseBody = StringUtils.trimToEmpty(response.body()); // Trim no body porque o webservice retorna
																			// espaços em branco antes do XML
		final var responseCode = response.statusCode();
		if (responseCode != 200) {
	...
	public NFSeSCSaoJoseCancelamentoRetorno enviarSolicitacaoCancelamentoNFSe(
			final NFSeSCSaoJoseCancelamentoSolicitacaoEnvio arquivoCancelamento, final String filename)
			throws Exception {
		HttpClient client = HttpClient.newHttpClient();
	...
	public NFSeSCSaoJosePesquisaCodigoAutenticidadeRetorno consultaNFSeCodigoAutenticidade(
			final NFSeSCSaoJosePesquisaCodigoAutenticidadeEnvio arquivoPesquisa, final String filename)
			throws Exception {
		HttpClient client = HttpClient.newHttpClient();
		this.getLogger().info("Request de consulta de NFSe com xml: {}", arquivoPesquisa.toXml());
		final var response = client.send(this.buildRequest(arquivoPesquisa.toXml(), filename),
				HttpResponse.BodyHandlers.ofString());
		final var responseBody = StringUtils.trimToEmpty(response.body());
		final var responseCode = response.statusCode();
		if (responseCode != 200) {
			throw new Exception(
					"Consulta de NFSe por codigo de autenticidade retornou erro '%d'!".formatted(responseCode));
		} else {
			final var registryMatcher = new NFSeRegistryMatcher();
			registryMatcher.bind(BigDecimal.class, new NFSeBigDecimalTransformer());
			this.getLogger().info("NFSe localizada: {}", responseBody);
			return new NFSePersister(true, registryMatcher).read(NFSeSCSaoJosePesquisaCodigoAutenticidadeRetorno.class,
					responseBody);
		}
	}
	...
	private HttpRequest buildRequest(final String xml, final String filename) throws Exception {
		...
		try (final var baos = new ByteArrayOutputStream()) {
			baos.write(
					"--%s%sContent-Disposition: form-data; name=\"file\"; filename=\"%s\"%sContent-Type: text/xml; charset=UTF-8%s%s"
							.formatted(boundary, CRLF, filename, CRLF, CRLF, CRLF).getBytes(StandardCharsets.UTF_8));
			...
			return HttpRequest.newBuilder().uri(URI.create(URL_PRODUCAO))
					.header("Content-Type", "multipart/form-data; boundary=" + boundary)
					.header("Authorization", "Basic %s".formatted(this.credentials))
					.POST(HttpRequest.BodyPublishers.ofByteArray(baos.toByteArray())).timeout(Duration.ofSeconds(10))
					.build();
		}
	}
}
